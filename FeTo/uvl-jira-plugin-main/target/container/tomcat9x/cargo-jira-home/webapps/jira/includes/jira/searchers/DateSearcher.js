define('jira/searchers/datesearcher/date-types', ['require'], function (require) {
     ...
     * @private
     */
    var _excludeList = ["js-dp-type-toggle", "js-val", "js-measurement", "js-clause", "js-start-date", "js-end-date", "js-start-range", "js-end-range", "assistive"];
    ...
    var _labelfyContent = function _labelfyContent(content, radioKey) {
        ...
        $content.each(function (index, element) {
            ...
            if ($element.attr("class") && !_excludeList.some(function (className) {
                return $element.attr("class").split(" ").includes(className);
            })) {
                /*
...
define('jira/searchers/datesearcher/date-searcher', ['require'], function (require) {
    ...
    return Backbone.View.extend({
        ...
        _renderErrors: function _renderErrors(errors, view) {
            var viewId = view.$el.data("date-type");
            var errorId = void 0;
            var $errorMessage = void 0;
            // There can only be one error message for one view at a time
            jQuery.each(errors, function (errorName, error) {
                if (!$errorMessage) {
                    errorId = (viewId || errorName) + "-error";
                    $errorMessage = jQuery("<div class='error' />").attr("id", errorId).text(error);
                }

                // If there is no ':' there will be undefined at index 1.
                var fieldName = errorName.split(":")[1];
                if (!fieldName) {
                    // We can't find the related input to link the message to,
                    // so just adding role="alert".
                    $errorMessage.attr("role", "alert");
                } else {
                    var $field = view.$el.find("[data-field-name='" + fieldName + "']");
                    $field.attr({
                        "aria-invalid": true,
        ...
        render: function render() {
            ...
            jQuery.each(this.views, function (i, view) {
                if (this.updateFromValues(values)) {
                    ...
                    if (!_.isEmpty(values.errors)) {
                        instance._renderErrors(values.errors, view);
                    }
                    return false;
                }
            });
            ...
            setTimeout(function () {
                var $invalidInput = instance.$el.find("[aria-invalid='true']").first();
