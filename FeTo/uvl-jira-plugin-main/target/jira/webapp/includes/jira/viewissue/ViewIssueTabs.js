 ...
 */
define("jira/viewissue/tabs", ["wrm/require", 'jira/util/logger', "jquery", "jira/jquery/deferred", "jira/util/data/meta", "jira/dialog/form-dialog", "jira/ajs/ajax/smart-ajax", "jira/userhover/userhover", "jira/viewissue/tabs/analytics"], function (WRMRequire, logger, jQuery, jQueryDeferred, Meta, FormDialog, SmartAjax, Userhover, Analytics) {
    'use strict';
     ...
     */
    var AJAX_LINK_SELECTOR = '.' + AJAX_LOAD_CLASS;
    ...
    function bindToTabDivs(container) {
        ...
        var $newTabContents = getIssueActionsContainer();
    ...
    function enableAjaxOnLinks(context) {
        ...
        jQuery(context).find(AJAX_LINK_SELECTOR).click(function (event) {
            document.getElementById('activitymodule').classList.add('updating');
            var $trigger = jQuery(this);

            var isSortingButton = $trigger.hasClass('issue-activity-sort-link');
            var currentTab = jQuery('#issue-tabs .active-tab').data('id');

            if (isSortingButton) {
                WRMRequire('wr!com.atlassian.jira.jira-frontend-plugin:entrypoint-activityTabs').then(function () {
                    // eslint-disable-next-line @atlassian/module-checks/no-conditional-require
                    require('jira/activity-tabs/items-lazy-loader').then(function (itemsLazyLoader) {
                        var isTabRegisteredToSupportNewSorting = itemsLazyLoader.getRegisteredTabsIds().includes(currentTab);

                        if (!isTabRegisteredToSupportNewSorting) {
                            handleLoading($trigger, activeTabKey, event);
                        } else {
                            document.getElementById('activitymodule').classList.remove('updating');
                        }
                    });
                });
            } else {
    ...
    function handleLoading($trigger, activeTabKey, event) {
        ...
        handleAjaxContentsLoading(activeTabKey, $trigger.data('ajax') || $trigger.attr('href')).done(function ($container) {
            document.getElementById('activitymodule').classList.remove('updating');
    ...
    function handleAjaxContentsLoading(activeTabKey, loadingUrl) {
        ...
        var xhr = SmartAjax.makeRequest({
            ...
            url: loadingUrl,
            timeout: null,
            complete: function complete(xhr, status, smartAjaxResult) {
                if (status !== 'abort') {
                    xhrInProgress = null;
    ...
    function smartUpdate(container, newElements) {
        // Update reference  to $tabContents to avoid stale reference
        $tabContents = getIssueActionsContainer();
    ...
    function getIssueActionsContainer() {
        return jQuery(CONTAINER_SELECTOR).find('#issue_actions_container');
    }
