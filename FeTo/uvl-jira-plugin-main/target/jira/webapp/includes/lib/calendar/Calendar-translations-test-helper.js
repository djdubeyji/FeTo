    ...
    return {
        runTests: function (locale, consts, requireMoment) {
            ...
            test("test calendar popup selects day by click ", function () {
                ...
                jQuery(".day-3", $calendar).click();
                jQuery(".confirm", $calendar).click();
            ...
            test("test calendar popup can't select empty date cells", function () {
                var date = new Date(2012, 1, 1);
                Calendar.setup({inputField: this.input, button: this.trigger, date: date, firstDay: 0});
                this.trigger.click();
                var $calendar = jQuery(".calendar.active");
                var $emptyCell = jQuery(".emptycell:first", $calendar);
                $emptyCell.click();

                equal($emptyCell.hasClass("selected"), false);
            });
        }
    }
});
