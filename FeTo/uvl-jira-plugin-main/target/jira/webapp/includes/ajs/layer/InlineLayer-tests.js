...
AJS.test.require(["jira.webresources:inline-layer", "jira.webresources:ajs-underscorejs-amd-shim"], function () {
    ...
    var assert = {
        dropdown: {
            ...
            isAbove: function isAbove($field, $layer, message) {
                message = message ? message : 'should have been placed above the trigger';
                equal($layer.offset().top + $layer.height(), $field.offset().top, message);
            },
    ...
    test('[WindowPositioning] should inverse positioning if inversion behaviour is toggled on', function () {
        this.$field.css({ left: 0, bottom: 10, width: 175 });
        var positioningController = new WindowPositioning();
        positioningController.setInversePositioningIfNotFit(true);
        var layer = this.makeLayer({ positioningController: positioningController, alignment: LayerConstants.LEFT });
        layer.show();
        assert.dropdown.isAbove(this.$field, layer.layer());
    });
    ...
    asyncTest("offset still works without window.name xss", function () {
