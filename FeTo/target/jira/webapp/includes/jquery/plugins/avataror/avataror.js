...
jQuery.fn.avataror = function (options) {
    ...
    var Assistive = require("jira/util/assistive");
    var formatter = require("jira/util/formatter");
    this.each(function () {
        var $this = $(this);
        var SHIFT_STEP_PX = 5;
        var ANNOUNCEMENT_DEBOUNCE_TIMEOUT_MS = 1000;
        var pressedArrowKeys = {};
        var isAltPressed = false;

        function isArrowKeyEvent(e) {
            return e.key.startsWith("Arrow");
        }

        function isArrowKeyPressed() {
            return Object.values(pressedArrowKeys).includes(true);
        }
        ...
        avataror.img.load(function () {
            ...
            avataror.marker.attr({
                tabindex: 0,
                "aria-label": formatter.I18n.getText("avatarpicker.cropper.aria.label"),
                "aria-describedby": "jira-avatar-cropper-keyboard-instructions"
            });
            ...
            avataror.marker.keydown(avataror.handleKeyDown);
            avataror.marker.keyup(avataror.handleKeyUp);
        ...
        avataror.handleKeyboardMove = function (e) {
            if (pressedArrowKeys.ArrowUp && avataror.y !== 0) {
                avataror.y -= e.shiftKey ? SHIFT_STEP_PX : 1;
                if (avataror.y < 0) {
                    avataror.y = 0;
                }
            } else if (pressedArrowKeys.ArrowDown && avataror.y + avataror.width !== avataror.imgheight) {
                avataror.y += e.shiftKey ? SHIFT_STEP_PX : 1;
                if (avataror.y + avataror.width > avataror.imgheight) {
                    avataror.y = avataror.imgheight - avataror.width;
                }
            }
            if (pressedArrowKeys.ArrowLeft && avataror.x !== 0) {
                avataror.x -= e.shiftKey ? SHIFT_STEP_PX : 1;
                if (avataror.x < 0) {
                    avataror.x = 0;
                }
            } else if (pressedArrowKeys.ArrowRight && avataror.x + avataror.width !== avataror.imgwidth) {
                avataror.x += e.shiftKey ? SHIFT_STEP_PX : 1;
                if (avataror.x + avataror.width > avataror.imgwidth) {
                    avataror.x = avataror.imgwidth - avataror.width;
                }
        ...
        avataror.handleKeyboardResize = function (e) {
            var sizeDelta = e.shiftKey ? SHIFT_STEP_PX : 1;
            if (pressedArrowKeys.ArrowLeft || pressedArrowKeys.ArrowUp) {
                sizeDelta = -sizeDelta;
            }

            var newSize = avataror.width + sizeDelta;

            if (newSize < 20) {
                newSize = 20;
            }
            if (avataror.x + newSize > avataror.imgwidth) {
                newSize = avataror.imgwidth - avataror.x;
            }
            if (avataror.y + newSize > avataror.imgheight) {
                newSize = avataror.imgheight - avataror.y;
            }

            avataror.width = avataror.height = newSize;
        };

        avataror.handleKeyDown = function (e) {
            if (e.key === "Alt") {
                isAltPressed = true;
            }
            ...
            pressedArrowKeys[e.key] = true;
            if (isAltPressed) {
                avataror.handleKeyboardResize(e);
            } else {
                avataror.handleKeyboardMove(e);
            }
        ...
        avataror.announceChange = _.debounce(function () {
            var announcement = formatter.I18n.getText("avatarpicker.cropper.change.announcement.text", avataror.width, avataror.width, avataror.x, avataror.y, avataror.imgwidth, avataror.imgheight);
            Assistive.readText(announcement);
        }, ANNOUNCEMENT_DEBOUNCE_TIMEOUT_MS);

        avataror.handleKeyUp = function (e) {
            if (e.key === "Alt") {
                isAltPressed = false;
            }
            if (!isArrowKeyEvent(e)) {
            ...
            pressedArrowKeys[e.key] = false;
            if (!isArrowKeyPressed()) {
                avataror.announceChange();
                avataror.shadow.show();
                $("#avatar-offsetX").val(avataror.x);
                $("#avatar-offsetY").val(avataror.y);
                $("#avatar-width").val(avataror.width);
        ...
        avataror.nwHandler = function (dx, dy) {
            ...
            var diffX = Math.abs(tmpBase.x - anchor.x),
                diffY = Math.abs(tmpBase.y - anchor.y);
