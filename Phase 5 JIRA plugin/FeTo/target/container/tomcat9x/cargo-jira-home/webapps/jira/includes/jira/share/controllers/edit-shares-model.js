define('jira/share/controllers/edit-shares-model', ['aui/message', 'jquery', 'jira/util/formatter'], function (AuiMessages, jQuery, formatter) {
    'use strict';
    ...
    EditSharesModel.prototype = {
        ...
        improveSelectArialabel: function improveSelectArialabel(selector, groupType) {
            var select = selector.closest('fieldset').find('.js-default-single-group-picker');
            var selectBaseAriaLabel = select.attr('aria-label');
            select.attr('aria-label', selectBaseAriaLabel + ' ' + groupType);
        },
        ...
        getForm: function getForm() {
            if (typeof jQuery('body').data('loaded') === 'undefined') {
                jQuery('body').attr('data-loaded', 'loaded');
                var viewersSelect = jQuery('#share_type_selector_viewers');
                var editorsSelect = jQuery('#share_type_selector_editors');
                if (viewersSelect.length) {
                    this.improveSelectArialabel(viewersSelect, formatter.I18n.getText('dashboard.create.select.group.viewers'));
                }
                if (editorsSelect.length) {
                    this.improveSelectArialabel(editorsSelect, formatter.I18n.getText('dashboard.create.select.group.editors'));
                }
            }
