define('jira/searchers/element/version-picker-options', ['wrm/context-path', 'jira/util/formatter', 'jira/ajs/list/item-descriptor', 'jira/ajs/list/group-descriptor', 'jira/searchers/element/common-pickers-config'], function (contextPath, formatter, ItemDescriptor, GroupDescriptor, commonPickersConfig) {
    'use strict';

    /**
     * @param {Object} baseAjaxOptions
     * @param {boolean} baseAjaxOptions.useIdPrefixInValue: Controls if to use version name as value (needed to accommodate both Create/Edit issue dialog and Basic issue search)
     * @param {number} baseAjaxOptions.maxResults: Page size
     * @param {number} baseAjaxOptions.startAt: Page start
    ...
    var getAjaxOptions = function getAjaxOptions(baseAjaxOptions) {
        ...
        return {
            url: contextPath() + '/rest/api/2/version',
            data: data,
            /**
