AJS.test.require('jira.webresources:projectpermissions', function () {
    ...
    function getMockPermissionSchemeModel(server) {
        ...
        server.requests[0].respond(200, {
            ...
            displayRendering: {
                readOnly: false,
                grouping: [ISSUE_PERMISSIONS_GROUP, PROJECT_PERMISSIONS_GROUP]
            },
            permissions: [ADD_COMMENT_PERMISSION, REMOVE_COMMENT_PERMISSION, CREATE_PROJECTS_PERMISSION]
        }));
    ...
    test('Ensure all permission groups are rendered correctly', function () {
        ...
        strictEqual(AJS.$('.project-permissions-category-header', permissionGroups[0]).text(), ISSUE_PERMISSIONS_GROUP.heading, 'First permission group should be issue permissions');

        strictEqual(AJS.$('.project-permissions-category-header', permissionGroups[1]).text(), PROJECT_PERMISSIONS_GROUP.heading, 'Second permission group should be project permissions');
    });
    ...
    test('Ensure permission group content is rendered correctly', function () {
        ...
        strictEqual(addCommentGrants.find('dd').text(), ADD_COMMENT_PERMISSION.grants[0].values[0].displayValue, 'Should have the administration application role grant');
    });
    ...
    module('Rendering feedback messages', {
        ...
        teardown: function teardown() {
            ...
            $('.aui-flag .aui-close-button').click();
            this.sandbox.restore();
        }
    });
    ...
    test('server response with multiple messages should render only the first one', function () {
        ...
        view.displayOperationSuccessMessage({
            operationResult: {
                type: 'success',
                messages: [messageText, 'This message should not be rendered in the Project Permissions page']
            }
        });
    ...
    function assertFlagPresenceByTitle(title) {
        var $flag = $('#aui-flag-container .aui-flag[open] .title:contains(' + title + ')');
    ...
    function assertFlagPresenceByBodyText(text) {
        var $flag = $('#aui-flag-container .aui-flag[open] .aui-message').contents().filter(function () {
    ...
    function getVisibleFlags() {
        return $('#aui-flag-container .aui-flag[open]');
    }
});