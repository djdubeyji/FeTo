AJS.test.require('jira.webresources:browseprojects', function () {
    ...
    require(['underscore', 'jquery', 'jira/backbone-1.3.3', 'jira/util/users/logged-in-user', 'jira/util/data/meta', 'jira/project/browse/app', 'jira/util/navigation'], function (_, $, Backbone, loggedInUser, meta, BrowseProjectsApp, navigation) {
        ...
        function generateProjects(number, categories) {
            ...
            for (var i = 1; i <= number; i++) {
                ...
                projects.push({
                    ...
                    archivedByUsername: i === 4 || i === 5 ? "admin" : undefined,
                    isArchivedByUserActive: i === 4 || i === 5 ? true : false,
        ...
        test('Sorting by default: column name should be sorted by ascending, all other - none', function () {
            this.inAdminMode = true;
            this.projects = [];
            this.startApplication("all");
            equal(this.$container.find('.project-list-name').attr("aria-sort"), 'ascending');
            equal(this.$container.find('.project-list-key').attr("aria-sort"), 'none');
            equal(this.$container.find('.project-list-type').attr("aria-sort"), 'none');
            equal(this.$container.find('.project-list-lead').attr("aria-sort"), 'none');
            equal(this.$container.find('.project-list-category').attr("aria-sort"), 'none');
        });

        test('Sorting by default: aria label should be presented for table header button', function () {
            this.inAdminMode = true;
            this.projects = [];
            this.startApplication("all");
            equal(this.$container.find('.project-list-name .sortable-button').attr("aria-label"), formatter.I18n.getText('common.concepts.sort', formatter.I18n.getText('common.concepts.project'), 'ascending', 'descending'));
            equal(this.$container.find('.project-list-key .sortable-button').attr("aria-label"), formatter.I18n.getText('common.concepts.sort.no', formatter.I18n.getText('common.concepts.key')));
            equal(this.$container.find('.project-list-type .sortable-button').attr("aria-label"), formatter.I18n.getText('common.concepts.sort.no', formatter.I18n.getText('browseprojects.project.type')));
            equal(this.$container.find('.project-list-lead .sortable-button').attr("aria-label"), formatter.I18n.getText('common.concepts.sort.no', formatter.I18n.getText('browseprojects.project.lead')));
            equal(this.$container.find('.project-list-category .sortable-button').attr("aria-label"), formatter.I18n.getText('common.concepts.sort.no', formatter.I18n.getText('browseprojects.project.category')));
        });
    });
});