define('jira/ajs/select/single-select', ['jira/util/formatter', 'jira/ajs/layer/layer-constants', 'jira/ajs/select/queryable-dropdown-select', 'jira/ajs/select/select-helper', 'jira/ajs/select/select-model', 'jira/ajs/select/suggestions/select-suggest-handler', 'jira/ajs/layer/inline-layer-factory', 'jira/ajs/list/list', 'jira/ajs/list/item-descriptor', 'jira/util/navigator', 'jquery'], function (formatter, LayerConstants, QueryableDropdownSelect, SelectHelper, SelectModel, SelectSuggestHandler, InlineLayerFactory, List, ItemDescriptor, Navigator, jQuery) {
    ...
    return QueryableDropdownSelect.extend({
         ...
         * @param {Boolean} [options.ariaLabel] -  Aria label for the form element.
         * @param {String} [options.ariaLabelledBy] - Aria-labelledby for the form element.
         * @param {('rounded'|'default')} [options.iconType='default'] - Entity icon type
         * @param {Boolean} [options.hasAddonIcon=false] - If true, adds space for custom addon icon or button on the right
        ...
        _prepareEvents: function _prepareEvents() {
            this._events = Object.assign(this._events, this.options.events);
        },
        ...
        _createFurniture: function _createFurniture(disabled) {
            ...
            if (disabled) {
                ...
                if (this.options.hasAddonIcon) {
                    this.$container.addClass("aui-ss-has-addon-icon");
                }
        ...
        _iconClass: function _iconClass(type) {
            if (type === 'rounded') {
                return 'rounded';
            }
            return '';
        ...
        setSelection: function setSelection(descriptor, triggerChangeEvent, event) {
            ...
            if (this._hasIcon()) {
                ...
                this.$entityIcon = this._render("entityIcon", descriptor.icon(), this.options.iconType).appendTo(this.$container);

                // JACCESS-1179 : Clone dropdown arrow behaviour to icon
                if (this._hasDropdownButton()) {
                    this._assignEvents("ignoreBlurElement", this.$entityIcon);
                    this._assignEvents("dropdownAndLoadingIcon", this.$entityIcon);
                }
            }
        ...
        _renders: {
            ...
            entityIcon: function entityIcon(url, type) {
                var icon = jQuery('<img class="aui-ss-entity-icon ' + this._iconClass(type) + '" alt=""/>');
                icon.attr("src", url);
                return icon;
            },
            field: function field(idPrefix, placeholder) {
                ...
                if (this.options.ariaLabel && !this.options.noAriaLabel) {
            ...
            dropdownAndLoadingIcon: function dropdownAndLoadingIcon(showDropdown) {
                ...
                return jQuery('<span class="icon aui-ss-icon noloading" tabindex="-1"></span>').toggleClass("drop-menu", !!showDropdown);
            }
        }
    });
});
